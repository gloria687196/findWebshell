...
def createHtml(resList):
	...
	for res in resList:
		...
		for ele in res:
			tmp += '<td>' + ele +'</td>'
		tr += '<tr>' + tmp + '</tr>'
	html = head + tr + '</table></div><h2 style="text-align:center">webshellHunter(findWebshell_plus_WDM-FDF by ZJW)</h2></html>'
	return html
